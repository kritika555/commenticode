...
//        prd($backlogs);
    ...
    public function addEdit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $token = $this->Common->formatTokenSend($this->request->data['amount'], false); die;
			$backlog = $this->Backlogs->patchEntity($backlog, $this->request->data);
            $jwtToken = $this->Common->getToken();
            $token = $this->Common->formatTokenSend($this->request->data['amount']);
            ...
            if ($this->Backlogs->save($backlog)) {
                ...
                //Create repo
                if($id == null){
              //$repoName = str_replace(' ', '-', $this->request->data['title']);
                    $repoName = uniqid();
                    $repoDesc = $this->request->data['description'];
					//error is here kritika
                    $repo = $this->Git->createRepo($repoName, $repoDesc);
                    $this->request->data['github_repo'] = json_encode($repo);
                }

                //Set backlog status = 1 if it is approved
                if($id == null || isset($this->request->query['status']) && $this->request->query['status'] == 'A'){
                    if(date('Y-m-d') == date('Y-m-d', strtotime($this->request->data['start_date']))) {
                ...
                if($id != null && $backlog->status == 'A' && $this->request->data['amount'] != $oldAmount){
                   // $url = Configure::read('ApiUrlDevAcc')."updateBacklogAmount/$id/$token/$jwtToken";
                    //$result = $this->Common->curlPost($url);
                }

                if($id == null || isset($this->request->query['status']) && $this->request->query['status'] == 'A'){
                    //$url = Configure::read('ApiUrlDevAcc')."addBacklog/$backlogId/$token/$jwtToken";
                   // $result = json_decode($this->Common->curlPost($url));
                    ...
                    if(isset($result->error->status) && $result->error->status == 0) {
                        ...
                        $this->Flash->error(__('The backlog has been saved in pending status, due to low balance.'));
                        return $this->redirect(['action' => 'index']);
                    }
                    //Update status to 1
                    if(date('Y-m-d') == date('Y-m-d', strtotime($this->request->data['start_date']))){
                        //$url = Configure::read('ApiUrlDevAcc')."updateBacklogStatus/$backlogId/$jwtToken";
                        //$result = $this->Common->curlPost($url);
                    }

                }

                $this->Flash->success(__('The backlog has been saved.'));
                return $this->redirect(['action' => 'index']);
            } else {
        ...
        $this->set(compact('backlog','action', 'userId'));
        $this->set('_serialize', ['backlog']);
    }
